...
const reply = (message: string, status = 200) => new Response(message, {
  status, headers: { 'Content-Type': 'text/plain; charset=utf-8', 'Cache-Control': 'no-store' },
});

export async function handleForm(request: Request, env: FormEnvironment, kind: 'contact' | 'newsletter', send: typeof fetch = fetch) {
  ...
  } catch { return reply('Invalid submission.', 400); }
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 320) return reply('Please provide a valid email address.', 400);
  if (kind === 'contact' && (!fullname || !phone || !message || fullname.length > 100 || phone.length > 50 || message.length > 5000)) {
    return reply('Please complete your name, phone number and message within the displayed limits.', 400);
  }
  if (kind === 'newsletter' && field('consent') !== 'yes') return reply('Please confirm that you want to receive the newsletter.', 400);
  ...
  try {
    ...
    if (!validation.ok || !(await validation.json() as { success?: boolean }).success) return reply('The security check expired or failed. Please try again.', 400);
    const response = await send(`https://api.resend.com/${kind === 'contact' ? 'emails' : 'contacts'}`, {
      ...
      body: JSON.stringify(kind === 'contact' ? {
        from: `TIA Interior <${env.SENDER_EMAIL}>`, to: [env.CONTACT_EMAIL], reply_to: email,
    ...
    return reply(kind === 'contact' ? 'Thank you! Your message has been sent.' : 'Thank you! You are subscribed to the TIA Interior newsletter.');
  } catch {
