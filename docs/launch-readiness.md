...
**Current release blocker (22 September 2026):** the live Homepage newsletter form omits its Turnstile container/public site key, so submissions reach the backend without a token. The repaired build renders the widget, exposes loading/error/expiry feedback, and keeps submit disabled until token callback. Local build/release/browser checks pass. Cloudflare build `7faf25f1-581b-459a-a9ed-37f37b2d4711` failed before deployment because the Git build environment lacked `PUBLIC_TURNSTILE_SITE_KEY`; the existing key is now configured there as a masked variable. The pipeline now runs `npm run build:release` once followed by `npx wrangler deploy`. Retrying production deployment and read-only live widget verification await explicit owner approval. No subscriber was created.
