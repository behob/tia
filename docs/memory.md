...
Last updated: 23 September 2026. Read [rules](rules.md), [PRD](prd.md) and relevant source before changes. This is a handoff, not a substitute for inspection.
...
- Font Awesome delivery now uses four local subset fonts containing the 26 glyphs referenced by source. The asset audit rejects newly referenced glyphs until the subset stylesheet/fonts are updated. Full vendor font files remain as unused source assets; built pages request only the subsets. The 23 September production audit confirmed 796–830 KiB lower transfer across the three measured pages with zero CLS and blocking time.
...
Production legacy compatibility redirects are live: all 26 configured variants returned direct 301 responses to exact destinations, and each destination returned 200 on 23 September. The focused [accessibility review](accessibility-review.md) found and resolved contrast, inactive-carousel exposure and generic image-alt issues; A11Y-002 through A11Y-004 are production-verified. Remaining business/editorial approval, live form tests, unavailable field data and product success targets are tracked in [tasks](tasks.md) and [PRD questions](prd.md#success-measures-and-product-decisions).
...
The [performance baseline](performance-baseline.md#mobile-follow-up-report-23-september-2026) now includes the owner's later post-commit PageSpeed report: homepage mobile performance 93, accessibility 97, LCP 3.2 s; desktop performance 100. Current local follow-up defers homepage Swiper CSS and lowers the below-fold about image's generated WebP from 88 to 58 KB. Browser interaction and build checks pass; no live score exists for this revision.

Latest verification: PERF-002 is deployed and the repeatable production mobile audit records 810 KiB for `/`, 271 KiB for the measured article and 289 KiB for `/contact`, down from 1,640, 1,085 and 1,119 KiB respectively. SEO-001 is complete: all 26 native Cloudflare redirects for slash variants of eight sector routes, two blog-template routes and three article aliases returned direct 301s to 200 destinations. A11Y-001 through A11Y-004 are deployed and production-verified: corrected contrast matches at 390px and 1440px, off-screen Swiper slides are inert and unfocusable in four named homepage regions, navigation updates exposed state, and no original generic homepage alternatives remain. Local build, lint, route, 95-route structural and browser interaction checks pass.

Prior work activated production form configuration without changing application code. Resend lists `tiadecors.com` as verified and permits Contacts access; the Cloudflare `tia` Worker now has `CONTACT_EMAIL`, `SENDER_EMAIL`, `RESEND_API_KEY`, `TURNSTILE_SECRET_KEY` and `NEWSLETTER_ENABLED`. Managed Turnstile is restricted to `tiadecors.com`, and the live public key matches. Non-sending live probes reached Turnstile for both endpoints. Cloudflare deployment `97dc96fa-0831-435a-9a2d-f895b258f6fd` routes 100% to version `33895136-6337-4ff2-bcb6-3edc6d37cba3`. No real email/subscriber test was performed.
...
FORM-001 is deployed and verified. Cloudflare Git build `7faf25f1-581b-459a-a9ed-37f37b2d4711` identified the missing build-time public key; the production trigger now stores it as a masked variable and runs one release build before Wrangler deployment. The form always renders the widget, disables submit until callback, exposes load/error/expiry states, retains the callback token independently of Cloudflare's hidden field and explicitly sends it. Release build, lint, mocked form and browser interaction checks passed. Production visibly reached Turnstile Success and enabled Subscribe on Worker version `96c4214d-773f-4de8-bb2b-f3b0f430e96d`. No subscriber was created; NEWS-002 remains the owner-authorized end-to-end subscription test.
