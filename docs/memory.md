...
Next: FORM-001 deployment and read-only live Turnstile verification. Live Homepage currently omits the Turnstile container/public key, causing tokenless newsletter requests. Local repair always renders the container, disables submit until callback, shows load/error/expiry states and passes built newsletter-token regression. Cloudflare Git build `7faf25f1-581b-459a-a9ed-37f37b2d4711` failed before deployment because its build environment lacked the public key. The production trigger now stores that key as a masked build variable and runs `npm run build:release` followed by `npx wrangler deploy`, avoiding the former duplicate build. A retry still needs explicit production deployment approval; no live subscriber was created. PERF-002 follows after this release blocker.
