...
- Font Awesome delivery now uses four local subset fonts containing the 26 glyphs referenced by source. The asset audit rejects newly referenced glyphs until the subset stylesheet/fonts are updated. Full vendor font files remain as unused source assets; built pages request only the subsets.
...
Latest implementation: PERF-002 is locally complete but awaits deployment measurement. `fontawesome-subset.css` and four WOFF2 subsets reduce referenced Font Awesome bytes from 1,623,793 to 7,526 (99.54%) without template changes. Build, lint, asset/glyph audit, blog desktop/mobile screenshots and interaction checks pass. No deployment occurred; keep PERF-002 `in_progress` until production transfer is measured. Next code task is SEO-001.
