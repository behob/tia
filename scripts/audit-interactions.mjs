...
try {
  ...
  for(const width of [1440,390]) {
    ...
    assert.equal(await page.getByRole('button',{name:'Open search dialog'}).evaluate(el=>el===document.activeElement),true);
    if(width===390) {
    ...
    await page.goto(base+'/contact',{waitUntil:'networkidle'});
    await page.route('**/api/mail',route=>route.fulfill({status:400,body:'Please complete the security check.'}));
    ...
    await page.getByRole('status').filter({hasText:'Please complete the security check.'}).waitFor().catch(async error => { console.log({errors,feedback:await page.locator('.form-feedback').allTextContents(),invalid:await page.locator(':invalid').evaluateAll(nodes=>nodes.map(n=>n.outerHTML))}); throw error; });
    assert.equal(await page.getByLabel('Full Name').inputValue(),'Test Visitor');
    await page.unroute('**/api/mail');
    await page.route('**/api/mail',route=>route.fulfill({status:200,body:'Thank you! Your message has been sent.'}));
    await page.getByRole('button',{name:'Send Message'}).click();
  ...
  const routes=['/','/spaces/apartments','/spaces/villas','/spaces/retail-spaces','/spaces/offices-workspaces','/spaces/restaurants-cafes','/spaces/hotels-resorts','/spaces/renovation-makeovers','/spaces/fit-out-custom-joinery','/blog-grid','/blog-list/2','/blog-standard/2'];
  ...
  for(const route of routes) {
    ...
    assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+2),false,`${route}: horizontal overflow`);
    await page.screenshot({path:`.astro/site-verification/${route==='/'?'home':route.replaceAll('/','_')}.png`,fullPage:false});
  }
  ...
  assert(schemas.flatMap(s=>JSON.parse(s)).some(s=>s['@type']==='BlogPosting'&&s.datePublished&&s.dateModified));
  ...
  console.log(`Interaction audit passed: desktop/mobile search, menus, contact feedback, headings, responsive images, categories, canonical URLs, reduced motion and no-JS rendering. Home legacy JS: ${legacy.length} files / ${bytes} bytes.`);
