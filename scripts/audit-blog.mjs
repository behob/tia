...
try {
  ...
  for (const width of [1440, 390]) {
    ...
    for (const view of ['grid', 'list', 'standard']) {
      ...
      await page.screenshot({ path: `${screenshotDir}/${view}-${width}.png` });
      // Test real navigation through the segmented view selector.
      const nextView = view === 'grid' ? 'list' : view === 'list' ? 'standard' : 'grid';
      await page.locator(`.blog-view-toggle a[href="/blog-${nextView}"]`).click();
      await page.waitForURL(`**/blog-${nextView}`);
    }
  }
  ...
  assert.equal(await noJs.locator('#blog-posts .post-card').count(), posts.length);
  assert.equal(await noJs.locator('.blog-view-toggle a').count(), 3);
  await noJs.close();
  console.log(
    `Blog audit passed: ${posts.length} posts, three views, desktop/mobile, selector navigation, article metadata, and legacy redirects.`,
  );
