...
const valid = {
  ...
  message: 'Please discuss a living room project.',
  'cf-turnstile-response': 'test-token',
  consent: 'yes',
};
const request = (body = valid, options = {}) =>
  new Request('https://example.test/api/mail', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Origin: 'https://example.test', ...options.headers },
    body: typeof body === 'string' ? body : JSON.stringify(body),
  });
let calls = [];
const mock = async (url, options) => {
  calls.push([url, options]);
  return new Response(JSON.stringify(url.includes('siteverify') ? { success: true } : { id: 'mock-id' }));
};
const reset = () => {
  calls = [];
};
...
for (const [body, config, kind, status] of cases) {
  reset();
  assert.equal((await handleForm(request(body), config, kind, mock)).status, status);
  assert.equal(calls.length, 0);
}
reset();
assert.equal(
  (await handleForm(request(valid, { headers: { Origin: 'https://elsewhere.test' } }), env, 'contact', mock)).status,
  403,
);
assert.equal(calls.length, 0);
...
assert.equal(
  (
    await handleForm(request(), env, 'contact', async () =>
      ++attempt === 1 ? new Response('{"success":true}') : new Response('error', { status: 500 }),
    )
  ).status,
  502,
);
...
console.log(
  'Form tests passed: contact, newsletter, malformed input, limits, origin, honeypot, consent, config, Turnstile and provider failures. All outbound requests mocked.',
);
